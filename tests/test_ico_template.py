...
# Based on BoaFixtureTest:
# https://github.com/CityOfZion/neo-boa/blob/master/boa_test/tests/boa_test.py
# Which is in turned based WalletFixtureTestCase:
# https://github.com/CityOfZion/neo-python/blob/master/neo/Utils/WalletFixtureTestCase.py
class TestContract(BoaFixtureTest):
    ...
    # Wallet in ./fixtures/testwallet.db3
    # Password 'testpassword'
    # This wallet is set as the contract owner for now
    wallet_1_script_hash = UInt160(data=b'\xb9l<\xe4\x0ey\x9ac\x9c^mM\x82Sk\x88oB\x1f\x15')
    wallet_1_addr = 'AYgJKV5e69q9tjZLauS5bFKKZjG2Qe4PTU'
    ...
    def test_1_owner_deploy(self):
        ...
        # Should pass deploy with wallet 1 (is set as owner)
        ...
        # There are still no tokens sold to the public
        tx, results, total_ops, engine = TestBuild(out, ['tokens_sold', parse_param([bytearray(TOKEN_OWNER)])], self.GetWallet1(), '0705', '05')
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].GetBigInteger(), 0)


    # Transfer from owner to third party wallet
    ...
    def test_3_kyc(self):
        ...
        tx, results, total_ops, engine = TestBuild(out, ['tokensale_register', parse_param([self.wallet_3_script_hash.Data])], self.GetWallet3(), '0705', '05')
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].GetBoolean(), False)

        # Get status of non registered address
        tx, results, total_ops, engine = TestBuild(out, ['tokensale_status', parse_param([self.wallet_3_script_hash.Data])], self.GetWallet3(), '0705', '05')
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].GetBoolean(), False)
        ...
        # register an address
        tx, results, total_ops, engine = TestBuild(out, ['tokensale_register', parse_param([self.wallet_3_script_hash.Data])], self.GetWallet1(), '0705', '05')
        self.assertEqual(len(results), 1)
        ...
        tx, results, total_ops, engine = TestBuild(out, ['tokensale_register', parse_param([self.wallet_3_script_hash.Data, self.wallet_2_script_hash.Data])], self.GetWallet1(), '0705', '05')
        self.assertEqual(len(results), 1)
    ...
    # Test we can properly get NEO/GAS part of transaction
    def test_4_attachments(self):
    ...
    # Verify can pull from contract
    def test_5_mint(self):
